...
void imstack::createmask(int numsamples, float scalef)
{
	HSVstack hsvstack;
	...
	//mask
	cv::inRange(average_v, cv::Scalar(0), cv::Scalar(0.1), mask_h);
	cv::bitwise_not(mask_h, mask_h);
	//connected components filter
	cv::Mat labels, stats, centroids;
	int nlabels = cv::connectedComponentsWithStats(mask_h, labels, stats, centroids, 8);
...
void imstack::objpixelsv(Rectangle rectangle, int labelid, float limsimi)
{
	cv::Mat tempimg = final_label.clone();
	tempimg.setTo(0, (tempimg != labelid));
	tempimg.setTo(1, (tempimg == labelid));
	...
	for (int i = 0; i < temp.v[0].rows; i++)
	{
		for (int j = 0; j < temp.v[0].cols; j++)
		{
			...
			if (!(std::all_of(tempch_v.begin(), tempch_v.end(), [](float i) { return i == 0; }))) //save only vectors with at least a non zero element
			{
